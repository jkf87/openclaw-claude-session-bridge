...
async function run() {
    ...
    const bridge1 = new bridge_1.SessionBridge({ statePath: STATE_PATH });
    bridge1.init();
    ...
    assert(statusAfterSend.data.history.length === 4, "history has 4 messages (2 user + 2 assistant)");
    console.log();
    // -----------------------------------------------------------------------
    // Scenario 3: Resume from saved state
    // -----------------------------------------------------------------------
    console.log("Scenario 3: Resume from saved state (new bridge instance)");
    const bridge2 = new bridge_1.SessionBridge({ statePath: STATE_PATH });
    const resumed = bridge2.status(sessionKey);
    assert(resumed.ok === true, "resumed bridge finds session");
    assert(resumed.data.childSessionKey === sessionKey, "session key matches");
    assert(resumed.data.history.length === 4, "history is preserved");
    ...
    assert(missingStatus.error?.includes("not found") === true, "error message mentions not found");
    ...
    assert(exported.data.sessions.length >= 1, "exported config contains sessions");
    ...
    const bridge3 = new bridge_1.SessionBridge({ statePath: STATE_PATH2 });
    bridge3.init();
    const importResult = bridge3.importConfig(exported.data);
    assert(importResult.ok === true, "import succeeds");
    assert(importResult.data.imported >= 1, "imported at least 1 session");
